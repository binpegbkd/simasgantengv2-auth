...
class LoginForm extends Model
{
    ...
    public function validatePassword($attribute, $params)
    {
        if (!$this->hasErrors()) {
            ...
            if (!$user || !$user->validatePassword($this->password) || $_SESSION["__captcha/site/captcha"] != $this->verifyCode) {
                $this->addError($attribute, '');
                $err_msg = Yii::$app->session->setFlash('position', [
                    'icon' => \dominus77\sweetalert2\Alert::TYPE_ERROR, //SUCCESS, INFO, ERROR, WARNING, QUESTION
                    'title' => 'Login Gagal',
                    'text' => 'Username/ Password/ Kode Verifikasi Salah !!',
